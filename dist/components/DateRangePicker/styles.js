...
export const CalendarDay = styled.div `
  ...
  &:before {
    ...
    ${({ today, isFirstSelected, isLastSelected }) => today && !(isFirstSelected || isLastSelected) ? `border: 1px solid ${theme.colors.cyan40}` : ''};
  }
`;
export const CalendarDaySelectedBackground = styled.div `
  position: absolute;
  ...
  &::before {
    content: '';
    border-radius: 50%;
    background-color: ${({ isFirstSelected, isLastSelected }) => isFirstSelected || isLastSelected ? theme.colors.cyan40 : 'transparent'};
    position: absolute;
    inset: 0;
  }
`;
