...
export const Label = styled.label<props>`
  ...
  span {
    ...
    border: 1px solid ${({ $disabled }) => (!$disabled ? theme.colors.cyan20 : theme.colors.shade30)};
  ...
  input:checked + span {
    cursor: pointer;
    border-radius: 2px;
    background-color: ${({ $disabled }) => (!$disabled ? theme.colors.cyan30 : theme.colors.shade20)};
